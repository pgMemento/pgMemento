...
BEGIN
  ...
  CREATE TABLE citydb.test (
    id SERIAL,
    test_column TEXT,
    test_geom_column geometry(PointZ,4326)
  );
...
BEGIN
  ...
  ASSERT defaults[1] = 'nextval(''citydb.test_id_seq''::regclass)', 'Incorrect default value for serial column ''id'' in audit_column_log: %', defaults[1];  
